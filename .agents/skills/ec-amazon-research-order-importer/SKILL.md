...
- After pasting `Code.paste.gs`, run `repairAll` once to fix existing B/C column duplicates, then use `importAmazonOrderEmails` for normal operation.
- Current duplicate prevention rule: new rows from `importAmazonOrderEmails` and `reprocessReviewRowsFromGmail` must run B/C cell duplicate repair immediately after insertion.
- B column duplicate key is normalized SKU + normalized sales amount digits only. C column duplicate key is normalized search-word block text.
