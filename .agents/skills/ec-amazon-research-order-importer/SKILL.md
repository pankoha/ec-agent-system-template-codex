...
## 2026-07-01 Update: Immediate research after Gmail import

- When `importAmazonOrderEmails` appends new rows to `注文確定商品リサーチ表`, immediately synchronize only those newly imported order numbers to `リサーチ管理表`.
- After synchronization, start research for only the newly imported orders in the same execution. Do not re-research all visible existing rows from this path.
- `researchListedItemsHourly` must research visible rows in `リサーチ管理表` after synchronizing from `注文確定商品リサーチ表`; do not accidentally research the main sheet as the primary hourly target.
- Preserve manual user work: do not overwrite existing URLs, hand-written statuses, notes, hidden rows, deleted columns, colors, or formatting.
- After the first candidate is recorded, append later candidates only when they improve the row: a lower comparable total price, or a better condition rank. Do not append worse candidates only because their URL is new.
- If the management sheet is missing, hidden, or has duplicate rows for the same order number, record the issue in `確認用` and keep the main import flow running.
- Use the order number as the only key for this immediate import-to-management-to-research chain.

## 2026-07-01 Update: Order-number synchronization

- Treat the order number as the only key shared by `注文確定商品リサーチ表` and `リサーチ管理表`.
- `researchListedItemsHourly` synchronizes from the main sheet first, then researches visible rows in the management sheet. Candidate URL appends must be written only to `リサーチ管理表`, never to the main order sheet.
- `リサーチ管理シート` is a legacy/unused sheet name. During setup, delete it if present; keep `リサーチ管理表` and `削除済み注文`.
...
- A column = 注文日 / 出荷予定日. Cell content must mirror the main sheet two-line value.
...
- I column = メルカリ.
- J column = ジモティ.
- K column = その他サイト only for legacy layouts. New research-management sheets use J column = 楽天市場 and K/L for 最終リサーチ日時/確認メモ.
...
- When sorting by 注文日, sort through `getLastColumn()` instead of a fixed A:K range so manually entered research/status/URL fields stay attached to the same order row.
...
- `リサーチ管理表` A column must copy the same wrapped A-column value from `注文確定商品リサーチ表`.
